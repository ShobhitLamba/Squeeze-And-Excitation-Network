# ISqueeze and Excite Inception-ResNet-v1 architecture
...
from se_inception_resnet_v1 import se_reduction_resnet_A

from se_block import squeeze_excite_block

def se_resnet_v2_stem(input):
...
def se_inception_resnet_v2_A(input, scale_residual = True):
...
def se_inception_resnet_v2_B(input, scale_residual = True):
...
def se_inception_resnet_v2_C(input, scale_residual = True):
...
def se_reduction_resnet_v2_B(input):
...
def se_inception_resnet_v2(nb_classes = 1001, scale = True):
    ...
    x = se_resnet_v2_stem(init) # Output: 35 * 35 * 256
    
     # 5 x Inception A
    for i in range(5):
        x = se_inception_resnet_v2_A(x, scale_residual = scale)
        # Output: 35 * 35 * 256
        
    # Reduction A
    x = se_reduction_resnet_A(x, k = 256, l = 256, m = 384, n = 384) # Output: 17 * 17 * 896

    # 10 x Inception B
    for i in range(10):
        x = se_inception_resnet_v2_B(x, scale_residual = scale)
        # Output: 17 * 17 * 896
        
    # Reduction B
    x = se_reduction_resnet_v2_B(x) # Output: 8 * 8 * 1792

    # 5 x Inception C
    for i in range(5):
        x = se_inception_resnet_v2_C(x, scale_residual = scale) 
        # Output: 8 * 8 * 1792
    ...
    model = Model(init, output, name = "SE-Inception-ResNet-v2")   
        
    return model


if __name__ == "__main__":
    se_inception_resnet_v2 = se_inception_resnet_v2()
    se_inception_resnet_v2.summary()
